...
?><table class="forum-reviews-messages" cellpadding="0" cellspacing="0" width="100%" border="0"><?
	...
	if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y"):?>
				<td><a href="#review_anchor" onMouseDown="quoteMessageEx('<?=$res["FOR_JS"]["AUTHOR_NAME"]?>', 'message_text_<?=$res["ID"]?>')" title="<?=GetMessage("FTR_QUOTE_HINT")?>" class="button-small"><?=GetMessage("FTR_QUOTE_FULL")?></a></td><?
	endif;
...
<textarea name="REVIEW_TEXT" id="REVIEW_TEXT" tabindex="<?=$tabIndex++;?>" cols="60" rows="10"><?=$arResult["REVIEW_TEXT"];?></textarea><br /><?
