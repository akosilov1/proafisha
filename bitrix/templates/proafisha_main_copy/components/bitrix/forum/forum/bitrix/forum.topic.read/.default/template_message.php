...
function __forum_default_template_show_message($arResult, $message, $arRes, $arAddParams, $arParams, $component = false)
{
	...
	if ($arParams["SHOW_RATING"] == 'Y'):
		...
		if (!empty($arAuthorId))
		{
			if (!is_array($arParams["RATING_ID"]))
				$arParams["RATING_ID"] = array($arParams["RATING_ID"]);
...
?>
								<span class="forum-action-hide"><?
									?><noindex><a rel="nofollow" <?if ($arParams['AJAX_POST'] == 'Y') { ?>onclick="return forumActionComment(this, 'MODERATE');"<? } ?> href="<?=$res["URL"]["MODERATE"]?>"><?=GetMessage("F_HIDE")?></a></noindex></span>&nbsp;&nbsp;
<?
						else:
?>
								<span class="forum-action-show"><?
									?><noindex><a rel="nofollow" <?if ($arParams['AJAX_POST'] == 'Y') { ?>onclick="return forumActionComment(this, 'MODERATE');"<? } ?> href="<?=$res["URL"]["MODERATE"]?>"><?=GetMessage("F_SHOW")?></a></noindex></span>&nbsp;&nbsp;
<?
						endif;
					endif;
					if ($res["PANELS"]["DELETE"] == "Y"):
?>
						...
						if ($arParams['AJAX_POST'] == 'Y') {
							...
							?>onclick="return confirm('<?=GetMessage('F_DELETE_MESSAGES_CONFIRM')?>');"<?
						} ?> href="<?=$res["URL"]["DELETE"]?>" <?
